/**
 * Type-level tests for tagged-ts (positional constructors).
 *
...
// Reusable spec shapes for Constructors / TaggedUnion structural tests
type MaybeSpecShape = {
  readonly Just: readonly ['value']
  readonly Nothing: readonly []
}
type StreamSpecShape = {
...
// ===========================================================================
// MemberSpec — positional field-name tuples
// ===========================================================================

describe('MemberSpec (positional — field name tuples)', () => {
  test('Nothing is nullary: readonly []', () => {
    type MaybeMemberSpec = MemberSpec<MaybeLambda>
    expectTypeOf<MaybeMemberSpec['Nothing']>().toEqualTypeOf<readonly []>()
  })

  test('CounterAction Reset is nullary with custom DK', () => {
    type CounterSpec = MemberSpec<CounterActionLambda, 'type'>
    expectTypeOf<CounterSpec['Reset']>().toEqualTypeOf<readonly []>()
  })

  test('Stream Done is nullary at arity 4', () => {
    type StreamSpec = MemberSpec<StreamLambda>
    expectTypeOf<StreamSpec['Done']>().toEqualTypeOf<readonly []>()
  })
})
...
describe('Constructors structural shape', () => {
  test('Nullary Nothing is not a function, non-nullary Just is', () => {
    type MaybeCtors = Constructors<MaybeLambda, 'tag', MaybeSpecShape>
  ...
  test('Arity-4: Done is not a function, Emit is', () => {
    type StreamCtors = Constructors<StreamLambda, 'tag', StreamSpecShape>
...
describe('Constructors — correct usage', () => {
  ...
  test('Positional constructor infers generic type param', () => {
  ...
  test('Arity-0 CounterAction: Increment takes positional arg, Reset is a constant', () => {
  ...
  test('Arity-4 Stream constructors (multi-field positional)', () => {
...
describe('Constructors — invalid usage rejected', () => {
  ...
  test('Result rejects missing positional args', () => {
  ...
  test('Env / Stream reject missing positional args', () => {
...
describe('Type guards', () => {
  test('guards return booleans', () => {
    expectTypeOf(Maybe.is.Just(Maybe.Just(1))).toEqualTypeOf<boolean>()
    expectTypeOf(Maybe.is.Nothing(Maybe.Nothing)).toEqualTypeOf<boolean>()
    expectTypeOf(Maybe.is.memberOfUnion(Maybe.Just(1))).toEqualTypeOf<boolean>()
  })

  test('Maybe.is.Just narrows to Just<number>', () => {
    const m = Maybe.Just(1) as Maybe<number>
    if (Maybe.is.Just(m)) {
  ...
  test('Result guards narrow exhaustively', () => {
    const r = Result.Success(1) as Result<string, number>
...
describe('match', () => {
  test('returns the handler return type', () => {
    const matchStr = Maybe.match(Maybe.Just(42), {
      Just: x => String(x.value),
      Nothing: _x => 'nothing',
    })
    expectTypeOf(matchStr).toEqualTypeOf<string>()

    const matchNum = Maybe.match(Maybe.Just(42), {
      Just: x => x.value,
      Nothing: _x => 0,
    })
...
describe('matchOr', () => {
  ...
  test('empty handler map falls through to the default', () => {
    const matchOrEmpty = Maybe.matchOr(Maybe.Just(42), {}, _otherwise => -1)
...
describe('matcher', () => {
  test('returns a function of the union type', () => {
    ...
    const applied = maybeMatcher(Maybe.Just(42))
...
describe('matcherW', () => {
  test('allows different return types per handler', () => {
    ...
    const applied = maybeMatcherW(Maybe.Just(42))
...
describe('memberOfUnion', () => {
  test('narrows unknown to Maybe tag union', () => {
...
describe('TaggedUnion structural type', () => {
  test('has correctly-typed feature fields, not just "present"', () => {
    type TU = TaggedUnion<MaybeLambda, 'tag', MaybeSpecShape>
...
describe('show (v0.6)', () => {
  ...
  test('accepts a value of the union type', () => {
    expectTypeOf(Maybe.show(Maybe.Just(1))).toEqualTypeOf<string>()
    expectTypeOf(
      Result.show(Result.Success<string, number>(1)),
    ).toEqualTypeOf<string>()
  })

  test('rejects cross-union arguments', () => {
    // @ts-expect-error - Result value is not a Maybe
    Maybe.show(Result.Success<string, number>(1))
  })
})
...
describe('equals (v0.6)', () => {
  test('Maybe.equals takes two Maybe<A> and returns boolean', () => {
    const eq = Maybe.equals(Maybe.Just(1), Maybe.Just(2))
  ...
  test('Result.equals threads both type parameters', () => {
    const eq = Result.equals(
      Result.Success<string, number>(1),
      Result.Failure<string, number>('e'),
    )
    expectTypeOf(eq).toEqualTypeOf<boolean>()
  })
